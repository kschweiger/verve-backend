...
def upgrade() -> None:
    """Upgrade schema."""
    op.execute("CREATE SCHEMA IF NOT EXISTS verve")
    ...
    op.execute("GRANT USAGE ON SCHEMA verve TO verve_user")
    op.execute(
        "ALTER DEFAULT PRIVILEGES IN SCHEMA verve GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO verve_user"  # noqa: E501
    )
    op.execute(
        "ALTER DEFAULT PRIVILEGES IN SCHEMA verve GRANT USAGE ON SEQUENCES TO verve_user"  # noqa: E501
    )
    ...
    for prefix, table in [
        ...
        op.execute(f"ALTER TABLE verve.{table} ENABLE ROW LEVEL SECURITY")
        op.execute(f"""
            CREATE POLICY {prefix}_isolation_policy ON verve.{table}
